...
func FetchVersions(a app.App, l *login.LoginInfo) ([]Version, error) {
	...
	response, err := nestorclient.CallAPI(fmt.Sprintf("/teams/%s/apps/%d/versions", l.DefaultTeamId, a.Id), "GET", params, 200)
	if err != nil {
		return versions, err
	}
...
func (v *Version) Deploy(a app.App, l *login.LoginInfo) error {
	...
	_, err := nestorclient.CallAPI(fmt.Sprintf("/teams/%s/apps/%d/deploys", l.DefaultTeamId, a.Id), "POST", params, 201)
	if err != nil {
		return err
	}
